...
def imProcessing(image_ocv, depth_data_ocv, visual=False, original_image=None, green=False):
    ...
    for gate in findGates.result:
        ...
        if redLine and yellowLine:  # only on correct readings
            target = (int((yellowLine[0] + redLine[0]) / 2),
                      int((yellowLine[1] + redLine[1]) / 2))
    ...
    reading = None
    if len(targetList):
        # averages a reading every 15 frames
        reading = calculateReading(targetList[0][0])

    if visual:
        if len(targetList):
            cv2.circle(original_image[startFrom:startFrom +
                                      pixelStrip], targetList[0], 5, (255, 0, 0), 4)
            cv2.line(original_image[startFrom:startFrom + pixelStrip],
                     targetList[1], targetList[2], (0, 255, 0), 10)
            if len(targetList) > 3:
                cv2.circle(
